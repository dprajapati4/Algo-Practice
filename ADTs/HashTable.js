...
//In order to look up a value by its key in an array we need a way to convert the keys into valud indices.
// A hash function performs this task. It takes an input and when you pass that to the hash function it return the same number/value everytime that same input is passed to the hash function.
// A good hash function is fast(constant time), distrubtes things uniformly so outputs are not clustered, and deterministic(one input === one output)
...
class HashTable {
  ...
  // A simpe hash function that converts strings to their charcode number 
  _hash(key) {
    let total = 0;
    let WEIRD_PRIME = 31;
